...
@section('content')
    <div class="w-75 d-block mx-auto my-5 p-4">
        ...
        <form method="POST" class="container bg-light shadow p-3 mb-5 bg-body-tertiary rounded"
            ...
            <h3 class="text-center my-5">Edit Data Staff</h3>
            @csrf
            @method('PUT')
            <div class="mb-3 mx-2">
            ...
            <div class="mb-3 mx-2">
                ...
                @enderror"
                    id="name" name="email" value="{{ $user['email'] }}">
                @error('email')
                    <small class="text-danger">{{ $message }}</small>
                @enderror
            </div>
            <div class="mb-3 mx-2">
                ...
                <input type="password" class="form-control @error('password') is-invalid @enderror" id="name" name="password" >
                @error('password')
                    <small class="text-danger">{{ $message }}</small>
                @enderror
            </div>
            <div class="d-grid gap-2">
                <button class="btn btn-primary mx-2 my-2" type="submit">Edit Data</button>
            </div>
        </form>
    </div>
@endsection
