...
<div class="container mt-5">
    ...
    <form class="w-100 d-block mx-auto my-3 card shadow-sm rounded p-3 bg-body" method="POST" action="{{ route("admin.users.store") }}">
        <h5 class="text-center">Buat Data Staff</h5>
        @csrf
        @error('name')
        ...
        <div data-mdb-input-init class="mb-3">
            ...
            <input type="text" id="form1Example1" class="form-control @error('name') is-invalid @enderror" name="name" >
        </div>
        @error('email')
            <small class="text-danger">{{$message}}</small>
        @enderror
        ...
        <button data-mdb-input-init type="submit" class="btn btn-primary btn-block">Buat</button>
